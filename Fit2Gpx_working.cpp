...
static bool ParseArgs(int argc, char** argv, Args& a) {
    ...
    for (int i = idx; i < argc; ++i) {
        ...
        else if (s == "--do-not-fetch-elevation") {
            a.fetchElevation = false;
        }
        else if (s == "--elevation-dataset" && (i + 1) < argc) {
            a.dataset_csv = argv[++i];
            std::string bad;
            if (!ValidateDatasetsCSV(a.dataset_csv, bad)) {
                std::cerr << "[args] Unknown dataset id: '" << bad << "'\n\n";
                PrintAllowedDatasetsDetailed();
                a.showHelp = true;
                return false;
            }
    ...
    if (a.gpxFile.empty()) a.gpxFile = DeriveGpxPath(a.fitFile);
    return true;
}
